...
if (!class_exists('DOTView')){
     ...
     *	    -
        ...
        public function init(){
            ...
            if (method_exists($DOT->controllers->$page,
                              'css')){
                $css = $DOT->controllers->$page->css();
                $dot_css['beta'] = isset($css['beta'])
                        ? array_merge($dot_css['beta'],
                                      $css['beta'])
                        : $dot_css['beta'];
                $dot_css['live'] = isset($css['live'])
                        ? array_merge($dot_css['live'],
                                      $css['live'])
                        : $dot_css['live'];
            }
            ...
            if (method_exists($DOT->controllers->$page,
                ...
                $dot_js['beta'] = isset($js['beta'])
                        ? array_merge($dot_js['beta'],
                                      $js['beta'])
                        : $dot_js['beta'];
                $dot_js['live'] = isset($js['live'])
                        ? array_merge($dot_js['live'],
                                      $js['live'])
                        : $dot_js['live'];
            }
        ...
        public function css(){
            ...
            foreach ($dot_css[DOT_STATUS] as $file){
                $path_sections = explode('/',
                                         $file);
                $file_name = str_replace('.css',
                                         '',
        ...
        public function js(){
            ...
            !wp_script_is('jquery-effects-core',
                    ...
                    : '';
            !wp_script_is('jquery-ui-datepicker',
                          'queue')
            ...
            foreach ($dopp_js[DOT_STATUS] as $file){
                $path_sections = explode('/',
                                         $file);
        ...
        public function minify($code){
            ...
            $search = array('/dot-css-assets-url\//'                                                                                                      => $DOT->paths->url,
                            // Add application URL in CSS files.
                            '!/\*.*?\*/!s'                                                                                                                => '',
                            // Remove comments.
                            '/ +/'                                                                                                                        => ' ',
                            // Remove empty spaces.
