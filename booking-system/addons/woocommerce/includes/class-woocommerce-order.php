...
* Version                 : 2.1.8
* File                    : addons/woocommerce/includes/class-woocommerce-order.php
* File Version            : 1.0.1
...
if (!class_exists('DOPBSPWooCommerceOrder')){
    class DOPBSPWooCommerceOrder{
        ...
        function __construct(){
             ...
             */
            add_action('woocommerce_order_status_partially-paid',
                       array(&$this,
                             'book'));
        ...
        function add($order,
            ...
            foreach ($cart_items as $cart_item_key => $values){
                ...
                if (isset($values['dopbsp_token'])){
                    ...
                    //phpcs:ignore WordPress.DB.DirectDatabaseQuery
                    $reservations_data = $wpdb->get_results($wpdb->prepare('SELECT * FROM %i WHERE cart_item_key=%s AND token=%s',
                    ...
                    foreach ($reservations_data as $reservation_data){
                        ...
                        $DOPBSP->classes->translation->set(defined('ICL_LANGUAGE_CODE')
                                                                   ? ICL_LANGUAGE_CODE
        ...
        function book($order_id){
            ...
            //phpcs:ignore WordPress.DB.DirectDatabaseQuery
            $reservations = $wpdb->get_results($wpdb->prepare('SELECT * FROM %i WHERE transaction_id=%d',
        ...
        function getDetails($reservation,
            ...
            $info[] = '<table class="dopbsp-wc-cart">';
            $info[] = '     <tbody>';
            ...
            if ($reservation->deposit_price>0){
                ...
                $info[] = $this->getInfo($DOPBSP->text('RESERVATIONS_RESERVATION_FRONT_END_DEPOSIT'),
                                         ($deposit->price_type == 'percent'
                                                 ? '<span class="info-rule">&#9632;&nbsp;'.$deposit->price.'%</span><br />'
                                                 : '').
        ...
        function getExtras($reservation,
            ...
            if (isset($reservation->extras)
                    && $reservation->extras != ''){
                $extras = is_string($reservation->extras)
                        ? json_decode($reservation->extras)
                        : $reservation->extras;
        ...
        function getDiscount($reservation,
            ...
            $discount = is_string($reservation->discount)
                    ? json_decode($reservation->discount)
