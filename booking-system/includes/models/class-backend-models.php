    ...
    class DOPBSPBackEndModels extends DOPBSPBackEnd{
        ...
        function display(){
            ...
            if ($wpdb->num_rows != 0){
                ...
                array_push($html,
                           '<li class="dopbsp-no-data">'.$DOPBSP->text('MODELS_NO_MODELS').'</li>');
            }
            array_push($html,
                       '</ul>');
        ...
        function listItem($model){
            ...
            array_push($html,
                       '<li class="dopbsp-item" id="DOPBSP-model-ID-'.$model->id.'" onclick="DOPBSPBackEndModel.display('.$model->id.')">');
            array_push($html,
             ...
             */
            array_push($html,
                       '     <span class="dopbsp-id">ID: '.$model->id.'</span>');
            ...
            if ($model->user_id>0){
                array_push($html,
                           '     <span class="dopbsp-header-item dopbsp-avatar">'.get_avatar($model->user_id,
                                                                                             17));
                array_push($html,
                           '         <span class="dopbsp-info">'.$DOPBSP->text('MODELS_CREATED_BY').': '.$user->data->display_name.'</span>');
                array_push($html,
            ...
            array_push($html,
                       ' </div>');
            array_push($html,
                       ' <div class="dopbsp-name">'.($model->name == ''
                               ? '&nbsp;'
                               : $model->name).'</div>');
            array_push($html,
                       '</li>');
