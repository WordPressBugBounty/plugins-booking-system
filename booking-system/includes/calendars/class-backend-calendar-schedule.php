     ...
     *
                    ...
                    if (!empty($events)){
                        ...
                        foreach ($reservations as $reservation){
                            ...
                            foreach ($events as $event){
                                $hours = $event->start->dateTime;
                                $start_hour = '';
                                $end_hour = '';
                                if (empty($hours)){
                                    $check_in = date('Y-m-d',
                                                     strtotime($event->start->date));
                                    $check_out = date('Y-m-d',
                                                      strtotime($event->end->date));
                                    ...
                                    $check_in_array = explode("+",
                                                              (string)$event->start->dateTime,
                                                              2);

                                    $check_in_date = $check_in_array[0];
                                    $check_in = date('Y-m-d',
                                                     strtotime($check_in_date));
                                    $start_hour_array = explode("T",
                                                                $check_in_date,
                                                                2);
                                    $start_hour = date('H:i',
                                                       strtotime($start_hour_array[1]));

                                    $timezone_diff = $check_in_array[1];

                                    $check_out_array = explode("+",
                                                               (string)$event->end->dateTime,
                                                               2);
                                    $check_out_date = $check_out_array[0];

                                    $check_out = '';
                                    $end_hour_array = explode("T",
                                                              $check_out_date,
                                                              2);
                                    $end_hour = date('H:i',
            ...
            if ($default_availability == 'false'){
                ...
                while ($data = current($schedule)){
                    ...
                    if ($hours_enabled == 'true'){
                        foreach ($data->hours as $hour):
                            $price = $hour->promo == ''
                                    ? ($hour->price == ''
                                            ...
                                            : (float)$hour->price)
                                    : (float)$hour->promo;
                            $available = $hour->available == '' && ($hour->status != 'booked' && $hour->status != 'none' && $hour->status != 'unavailable')
                                    ? ($hour->available == ''
                                            ? 0
                            ...
                            if ($hour->available != '0'){
                                $min_available = $available<$min_available
                                        ? $available
                                        : $min_available;
                            }
                        endforeach;
                        ...
                        $price_min = $data->promo == ''
                                ? ($data->price == ''
                                        ? 0
                                        : (float)$data->price)
                                : (float)$data->promo;
                        $price_max = $price_min;
                        $min_available = $data->available == '' && ($data->status != 'booked' && $data->status != 'none' && $data->status != 'unavailable')
                                ? ($data->available == ''
                                        ? 0
        ...
        function getDaysHoursSelected($day,
            ...
            foreach ($hours_definitions as $index => $hours_definition){
                if ($startHour<=$hours_definitions[$index]->value
                    ...
                    $found++;
                    array_push($selectedHours,
                               $hours_definitions[$index]->value);
                }
            }
        ...
        function daysHoursHistory($day,
            ...
            if (!empty($selectedHours)){
                //                $hours_length = count($selectedHours)-(!$settings_calendar->hours_multiple_select == 'true'
                //                                ? true
                //                                : ($settings_calendar->hours_add_last_hour_to_total_price == 'true' && $settings_calendar->hours_interval_enabled == 'false'
                //                                        ? true
                //                                        : false) || ($settings_calendar->hours_multiple_select == 'false'
                //                                        ? false
                //                                        : ($settings_calendar->hours_interval_enabled == 'true'
                //                                                ? true
                //                                                : false)) && ($settings_calendar->hours_multiple_select == 'true'
                //                                        ? true
                //                                        : false)
                //                                        ? 1
                //                                        : 0);

                $hours_length = count($selectedHours)-
                        ((!$settings_calendar->hours_multiple_select == 'true'
                                ? true
                                : ($settings_calendar->hours_add_last_hour_to_total_price == 'true' && $settings_calendar->hours_interval_enabled == 'false')
                                || ($settings_calendar->hours_multiple_select == 'false'
                                        ? false
                                        : $settings_calendar->hours_interval_enabled == 'true')
                                && $settings_calendar->hours_multiple_select == 'true')
                                ? 1
                ...
                for ($i = 0; $i<=$hours_length; $i++){
                    ...
                    if (!array_key_exists($currHour,
                                          (array)$history_all[$day]['hours'])){
                        $default = '{"available": 1,
                                         "bind": 0,
                                         "price": 0,
                                         "promo": 0,
                        ...
                        $default_history = $history_all[$day]['hours'][$currHour] != ''
                                ? $history_all[$day]['hours'][$currHour]
                                : json_decode($default);
        ...
        function daysHistory($ciDay,
            ...
            for ($i = 0; $i<count($selectedDays)-($settings_calendar->days_morning_check_out == 'true'
                    ? 1
                    : 0); $i++){
                $currDay = $selectedDays[$i];

                $history[$currDay] = $default_availability != ''
        ...
        function setApproved($reservation_id){
            ...
            $info_body = $this->getFormInfo(json_decode($reservation->form),
                                            'body');
            ...
            elseif ($reservation->end_hour != ''){
                ...
                //$reservation_end_hours = $DOPBSP->classes->prototypes->getLeadingZero((int)$reservation->end_hour);
        ...
        function setCanceled($reservation_id){
            ...
            if ($reservation->check_out == ''
                ...
                if ($data->status == 'booked'){
                    $data->available = $history->{$day_date}->available == ''
                            ? 'none'
                            : $data->available+$reservation->no_items;
                    $data->bind = (int)$history->{$day_date}->bind;
                    $data->price = (float)$history->{$day_date}->price;
                    $data->promo = (float)$history->{$day_date}->promo;
                    $data->status = $history->{$day_date}->status == null
                            ? 'none'
                            : $history->$day_date->status;
                }
                else{
                    if ($data->available != ''){
                        $data->available = $data->available+$reservation->no_items;
            ...
            elseif ($reservation->check_out != ''){
                ...
                foreach ($days as $key => $day){
                    ...
                    if ($data->status == 'booked'){
                        $data->available = $history->{$day_date}->available == ''
                                ? ''
                                : (int)$data->available+(int)$reservation->no_items;
                        $data->bind = (int)$history->{$day_date}->bind;
                        $data->price = (float)$history->{$day_date}->price;
                        $data->promo = (float)$history->{$day_date}->promo;
                        $data->status = $history->{$day_date}->status == null
            ...
            elseif ($reservation->start_hour != ''
                ...
                if ($hour->status == 'booked'){
                    $hour->available = $history->{$hour_time}->available == ''
                            ? ''
                            : $hour->available+$reservation->no_items;
        ...
        function validateDaysOverlap($calendar_id,
            ...
            for ($i = 0; $i<count($reservations); $i++){
                ...
                if ($this->validateDays($calendar_id,
                    ...
                    for ($j = 0; $j<count($selected_days)-($settings_calendar->days_morning_check_out == 'true'
                            ? 1
                            : 0); $j++){
        ...
        function validateHoursOverlap($calendar_id,
            ...
            for ($i = 0; $i<count($reservations); $i++){
                ...
                $end_hour = $reservations[$i]->end_hour == ''
                        ? $reservations[$i]->start_hour
                        : $reservations[$i]->end_hour;
                $no_items = $reservations[$i]->no_items;

                if ($this->validateHours($calendar_id,
                                         $day,
                                         $start_hour,
                                         $end_hour,
                                         $no_items)){
