...
if (!class_exists('DOPBSPBackEndSettingsUsers')){
    class DOPBSPBackEndSettingsUsers extends DOPBSPBackEndSettings{
                ...
                switch (key($roles)){
                    case 'administrator':
                        get_option('DOPBSP_users_permissions_administrator') == ''
                                ? add_option('DOPBSP_users_permissions_administrator',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_ADMINISTRATORS)
                                : '';
                        get_option('DOPBSP_users_permissions_custom_posts_administrator') == ''
                                ? add_option('DOPBSP_users_permissions_custom_posts_administrator',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_CUSTOM_POSTS_ADMINISTRATORS)
                                : '';
                        break;
                    case 'author':
                        get_option('DOPBSP_users_permissions_author') == ''
                                ? add_option('DOPBSP_users_permissions_author',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_AUTHORS)
                                : '';
                        get_option('DOPBSP_users_permissions_custom_posts_author') == ''
                                ? add_option('DOPBSP_users_permissions_custom_posts_author',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_CUSTOM_POSTS_AUTHORS)
                                : '';
                        break;
                    case 'contributor':
                        get_option('DOPBSP_users_permissions_contributor') == ''
                                ? add_option('DOPBSP_users_permissions_contributor',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_CONTRIBUTORS)
                                : '';
                        get_option('DOPBSP_users_permissions_custom_posts_contributor') == ''
                                ? add_option('DOPBSP_users_permissions_custom_posts_contributor',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_CUSTOM_POSTS_CONTRIBUTORS)
                                : '';
                        break;
                    case 'editor':
                        get_option('DOPBSP_users_permissions_editor') == ''
                                ? add_option('DOPBSP_users_permissions_editor',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_EDITORS)
                                : '';
                        get_option('DOPBSP_users_permissions_custom_posts_editor') == ''
                                ? add_option('DOPBSP_users_permissions_custom_posts_editor',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_CUSTOM_POSTS_EDITORS)
                                : '';
                        break;
                    case 'subscriber':
                        get_option('DOPBSP_users_permissions_subscriber') == ''
                                ? add_option('DOPBSP_users_permissions_subscriber',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_SUBSCRIBERS)
                                : '';
                        get_option('DOPBSP_users_permissions_custom_posts_subscriber') == ''
                                ? add_option('DOPBSP_users_permissions_custom_posts_subscriber',
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_CUSTOM_POSTS_SUBSCRIBERS)
                                : '';
                        break;
                    default:
                        get_option('DOPBSP_users_permissions_'.key($roles)) == ''
                                ? add_option('DOPBSP_users_permissions_'.key($roles),
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_OTHERS)
                                : '';
                        get_option('DOPBSP_users_permissions_custom_posts_'.key($roles)) == ''
                                ? add_option('DOPBSP_users_permissions_custom_posts_'.key($roles),
                                             DOPBSP_CONFIG_USERS_PERMISSIONS_CUSTOM_POSTS_OTHERS)
                                : '';
                }
                next($roles);
            }
        ...
        function set($args = false){
            ...
            $calendar_id = $DOT->post('calendar_id',
                                      'int')
                    ? $DOT->post('calendar_id',
                                 'int')
                    : (int)$args['calendar_id'];

            if ($id == 0){
                update_option('DOPBSP_users_permissions_'.$slug,
                              $value);
            }
            else{
                if ($calendar_id == 0){
                    if (get_user_meta($id,
                                      'DOPBSP_permissions_'.$slug,
                                      true) == ''){
                        add_user_meta($id,
                                      'DOPBSP_permissions_'.$slug,
                                      $value,
