    ...
    class DOPBSPBackEndSmses extends DOPBSPBackEnd{
        ...
        function display(){
            ...
            if ($wpdb->num_rows != 0){
                ...
                array_push($html,
                           '<li class="dopbsp-no-data">'.$DOPBSP->text('SMSES_NO_SMS').'</li>');
            }
            array_push($html,
                       '</ul>');
        ...
        function listItem($sms){
            ...
            array_push($html,
                       '<li class="dopbsp-item" id="DOPBSP-sms-ID-'.$sms->id.'" onclick="DOPBSPBackEndSms.display('.$sms->id.')">');
            array_push($html,
             ...
             */
            array_push($html,
                       '     <span class="dopbsp-id">ID: '.$sms->id.'</span>');
            ...
            if ($sms->user_id>0){
                array_push($html,
                           '     <span class="dopbsp-header-item dopbsp-avatar">'.get_avatar($sms->user_id,
                                                                                             17));
                array_push($html,
                           '         <span class="dopbsp-info">'.$DOPBSP->text('SMSES_CREATED_BY').': '.$user->data->display_name.'</span>');
                array_push($html,
            ...
            array_push($html,
                       ' </div>');
            array_push($html,
                       ' <div class="dopbsp-name">'.($sms->name == ''
                               ? '&nbsp;'
                               : $sms->name).'</div>');
            array_push($html,
                       '</li>');
