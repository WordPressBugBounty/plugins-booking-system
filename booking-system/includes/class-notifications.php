        ...
        */
        ...
        function sendMail($email_to,
            ...
            for ($i = 0; $i<count($emails_cc); $i++){
                $headers_cc[] = $emails_cc_name[$i].(isset($emails_cc[$i])
                                ? ' <'.$emails_cc[$i].'>'
                                : '');
            }
            ...
            for ($i = 0; $i<count($emails_bcc); $i++){
                $headers_bcc[] = $emails_bcc_name[$i].(isset($emails_bcc[$i])
                                ? ' <'.$emails_bcc[$i].'>'
                                : '');
            }
            ...
            $headers[] = 'Reply-To: '.$email_name.' <'.$email_reply.'>'."\r\n";
            $headers[] = 'Cc: '.implode(',',
                                        $headers_cc)."\r\n";
            $headers[] = 'Bcc: '.implode(',',
                                         $headers_bcc);
            ...
            if (!mail($email_to,
                      ...
                      implode('',
                              $headers))){
                if ($is_test){
                    $DOT->echo($DOPBSP->text('SETTINGS_NOTIFICATIONS_TEST_ERROR'));
                    die();
                }
            }
            else{
        ...
        function sendMailer($email_to,
            ...
            for ($i = 0; $i<count($emails_bcc); $i++){
                $php_mailer->addBCC($emails_bcc[$i],
                                    $emails_bcc_name[$i] ?? '');
            }
            ...
            if (!$php_mailer->send()){
                if ($is_test){
                    $DOT->echo($DOPBSP->text('SETTINGS_NOTIFICATIONS_TEST_ERROR').'<br />',
                               'content',
                               ['br' => []]);
                    $DOT->echo('Mailer error: '.$php_mailer->ErrorInfo.'<br />',
                               'content',
                               ['br' => []]);
                    $DOT->echo($php_mailer->ErrorInfo);
                    die();
                }
                else{
                    $this->sendWPMail($email_to,
        ...
        function sendWPMail($email_to,
            ...
            if (!wp_mail($email_to,
                         ...
                         implode('',
                                 $headers))){
                if ($is_test){
                    $DOPBSP->text('SETTINGS_NOTIFICATIONS_TEST_ERROR');
                    die();
                }
                else{
                    $this->sendMail($email_to,
        ...
        function test(){
            ...
            $email_reply = $settings_notifications->email_reply == ''
                    ...
                    : $settings_notifications->email_reply;
            $email_name = $settings_notifications->email_name == ''
                    ? $settings_notifications->email_reply
                    : $settings_notifications->email_name;
