...
var DOPBSPBackEndExtra = new function(){
    ...
    this.display = function(id,
        ...
        language = language === undefined
                ? ($('#DOPBSP-extra-language')
                .val() === undefined
                        ? ''
                        : $('#DOPBSP-extra-language')
                        .val())
                : language;
        clearExtra = clearExtra === undefined
                ? true
                : false;
        language = clearExtra
                ? ''
        ...
        $.post(ajaxurl,
               ...
               function(data){
                   ...
                   DOPBSPBackEnd.toggleMessages('success',
                                                DOPBSPBackEnd.text('EXTRAS_EXTRA_LOADED'));
               })
         .fail(function(data){
    ...
    this.add = function(){
        DOPBSPBackEnd.clearColumns(2);
        DOPBSPBackEnd.toggleMessages('active',
                                     DOPBSPBackEnd.text('EXTRAS_ADD_EXTRA_ADDING'));

        $.post(ajaxurl,
               {
                   action: 'dopbsp_extra_add',
                   nonce : DOPBSP_user_nonce
               },
               function(data){
                   ...
                   .html(data);
                   DOPBSPBackEnd.toggleMessages('success',
                                                DOPBSPBackEnd.text('EXTRAS_ADD_EXTRA_SUCCESS'));
               })
         .fail(function(data){
             DOPBSPBackEnd.toggleMessages('error',
                                          data.status+': '+data.statusText);
    ...
    this.edit = function(id,
        ...
        switch (field){
            case 'name':
                $('#DOPBSP-extra-ID-'+id+' .dopbsp-name')
                .html(value === ''
                              ? '&nbsp;'
                              : value);
                break;
    ...
    this.delete = function(id){
        DOPBSPBackEnd.toggleMessages('active',
                                     DOPBSPBackEnd.text('EXTRAS_DELETE_EXTRA_DELETING'));

        $.post(ajaxurl,
               {
                   action: 'dopbsp_extra_delete',
                   id    : id,
                   nonce : DOPBSP_user_nonce
               },
               function(data){
                   ...
                   .animate({'opacity': 0},
                            ...
                            function(){
                                ...
                                if (data === '0'){
                                    $('#DOPBSP-column1 .dopbsp-column-content')
                                    .html('<ul><li class="dopbsp-no-data">'+DOPBSPBackEnd.text('EXTRAS_NO_EXTRAS')+'</li></ul>');
                                }
                                DOPBSPBackEnd.toggleMessages('success',
                                                             DOPBSPBackEnd.text('EXTRAS_DELETE_EXTRA_SUCCESS'));
                            });
               })
         .fail(function(data){
             DOPBSPBackEnd.toggleMessages('error',
