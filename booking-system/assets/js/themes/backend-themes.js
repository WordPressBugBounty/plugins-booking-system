...
var DOPBSPBackEndThemes = new function(){
               ...
               function(data){
                   ...
                   if (data === 'error'){
                       DOPBSPBackEnd.toggleMessages('error',
                                                    DOPBSPBackEnd.text('THEMES_LOAD_ERROR'));
                   }
                   else{
                       content = data.split(';;;;;;;');
                       ...
                       DOPBSPBackEnd.toggleMessages('success',
                                                    DOPBSPBackEnd.text('THEMES_LOAD_SUCCESS'));
                   }
               })
         .fail(function(data){
             DOPBSPBackEnd.toggleMessages('error',
    ...
    this.events = function(){
        $('#DOPBSP-inputs-themes-filters-tags .dopbsp-input-wrapper')
        .unbind('click');
        $('#DOPBSP-inputs-themes-filters-tags .dopbsp-input-wrapper')
        .bind('click',
              function(){
                  ...
                  if ($(this)
                  .attr('data-filter') === '.dopbsp-all'){
                      $('#DOPBSP-inputs-themes-filters-tags .dopbsp-input-wrapper')
                      .removeClass('dopbsp-selected');
                      $('#DOPBSP-inputs-themes-filters-tags .dopbsp-input-wrapper input[type="checkbox"]')
                      .removeAttr('checked');
                  }
                  else{
                      $('#DOPBSP-inputs-themes-filters-tags .dopbsp-input-wrapper:first-child')
                      .removeClass('dopbsp-selected');
                      $('#DOPBSP-inputs-themes-filters-tags .dopbsp-input-wrapper:first-child input[type="checkbox"]')
                      .removeAttr('checked');
                  }
    ...
    this.search = function(){
        var filters     = new Array(),
            i           = 0,
            searchRegex = new RegExp($('#DOPBSP-themes-search')
                                     .val(),
                                     'gi');
        ...
        $('#DOPBSP-themes-list .dopbsp-theme')
        .each(function(){
            i++;

            if ($(this)
            .text()
            .match(searchRegex) !== null){
