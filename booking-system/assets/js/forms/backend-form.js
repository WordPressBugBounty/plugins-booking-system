...
var DOPBSPBackEndForm = new function(){
    ...
    this.display = function(id,
        ...
        language = language === undefined
                ? ($('#DOPBSP-form-language')
                .val() === undefined
                        ? ''
                        : $('#DOPBSP-form-language')
                        .val())
                : language;
        clearForm = clearForm === undefined
                ? true
                : false;
        language = clearForm
                ? ''
        ...
        $.post(ajaxurl,
               ...
               function(data){
                   ...
                   DOPBSPBackEnd.toggleMessages('success',
                                                DOPBSPBackEnd.text('FORMS_FORM_LOADED'));
               })
         .fail(function(data){
    ...
    this.add = function(){
        DOPBSPBackEnd.clearColumns(2);
        DOPBSPBackEnd.toggleMessages('active',
                                     DOPBSPBackEnd.text('FORMS_ADD_FORM_ADDING'));

        $.post(ajaxurl,
               {
                   action: 'dopbsp_form_add',
                   nonce : DOPBSP_user_nonce
               },
               function(data){
                   ...
                   .html(data);
                   DOPBSPBackEnd.toggleMessages('success',
                                                DOPBSPBackEnd.text('FORMS_ADD_FORM_SUCCESS'));
               })
         .fail(function(data){
             DOPBSPBackEnd.toggleMessages('error',
                                          data.status+': '+data.statusText);
    ...
    this.delete = function(id){
        DOPBSPBackEnd.toggleMessages('active',
                                     DOPBSPBackEnd.text('FORMS_DELETE_FORM_DELETING'));

        $.post(ajaxurl,
               {
                   action: 'dopbsp_form_delete',
                   id    : id,
                   nonce : DOPBSP_user_nonce
               },
               function(data){
                   ...
                   .animate({'opacity': 0},
                            ...
                            function(){
                                ...
                                if (data === '0'){
                                    $('#DOPBSP-column1 .dopbsp-column-content')
                                    .html('<ul><li class="dopbsp-no-data">'+DOPBSPBackEnd.text('FORMS_NO_FORMS')+'</li></ul>');
                                }
                                DOPBSPBackEnd.toggleMessages('success',
                                                             DOPBSPBackEnd.text('FORMS_DELETE_FORM_SUCCESS'));
                            });
               })
         .fail(function(data){
             DOPBSPBackEnd.toggleMessages('error',
