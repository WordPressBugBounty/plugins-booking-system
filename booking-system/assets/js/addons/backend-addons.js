...
var DOPBSPBackEndAddons = new function(){
               ...
               function(data){
                   ...
                   if (data === 'error'){
                       DOPBSPBackEnd.toggleMessages('error',
                                                    DOPBSPBackEnd.text('ADDONS_LOAD_ERROR'));
                   }
                   else{
                       content = data.split(';;;;;;;');
                       ...
                       DOPBSPBackEnd.toggleMessages('success',
                                                    DOPBSPBackEnd.text('ADDONS_LOAD_SUCCESS'));
                   }
               })
         .fail(function(data){
             DOPBSPBackEnd.toggleMessages('error',
    ...
    this.category = function(key){
        $('#DOPBSP-addons-search')
        .val('');
        $('#DOPBSP-column2 .dopbsp-column-content .dopbsp-addon')
        .css('display',
             'block');

        if ($('#DOPBSP-inputs-button-addons-category-'+key)
        .parent()
        .hasClass('dopbsp-hide')){
            $('#DOPBSP-addons-category-'+key)
            .removeAttr('checked');
        }
        else{
            $('#DOPBSP-addons-category-'+key)
    ...
    this.search = function(){
        var searchRegex = new RegExp($('#DOPBSP-addons-search')
                                     .val(),
                                     'gi');
        ...
        $('#DOPBSP-column2 .dopbsp-addons-list')
        .each(function(){
            var key      = $(this)
                .parent()
                .attr('id')
                .split('DOPBSP-inputs-addons-category-')[1],
                noAddons = 0,
                $parent  = $('#DOPBSP-inputs-button-addons-category-'+key)
                .parent();
            ...
            if (noAddons === 0){
                if ($parent.hasClass('dopbsp-hide')){
                    ...
                    $('#DOPBSP-addons-category-'+key)
                    .removeAttr('checked');
                    $('#DOPBSP-inputs-addons-category-'+key)
                    .css('display',
                         'none');
                }
            }
